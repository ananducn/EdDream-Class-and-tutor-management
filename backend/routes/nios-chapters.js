...
router.get('/', auth, async (req, res, next) => {
  try {
    const { nios_university_subject_id, include_inactive } = req.query;
    if (!nios_university_subject_id) return res.status(400).json({ error: 'nios_university_subject_id is required.' });
    const rows = include_inactive === 'true'
      ? await sql`
          SELECT c.*, u.name AS created_by_name
          FROM nios_chapters c LEFT JOIN users u ON u.id = c.created_by
          WHERE c.nios_university_subject_id = ${nios_university_subject_id}
          ORDER BY c.chapter_order, c.title`
      : await sql`
          SELECT c.*, u.name AS created_by_name
          FROM nios_chapters c LEFT JOIN users u ON u.id = c.created_by
          WHERE c.nios_university_subject_id = ${nios_university_subject_id} AND c.is_active = true
          ORDER BY c.chapter_order, c.title`;
    res.json(rows);
  } catch (err) { next(err); }
});
...
async function niosChapterPlace(niosUniversitySubjectId) {
  const rows = await sql`
    SELECT s.name AS subject_name, u.name AS university_name
    FROM nios_university_subjects us
    JOIN nios_subjects s ON s.id = us.nios_subject_id
    JOIN nios_universities u ON u.id = us.nios_university_id
    WHERE us.id = ${niosUniversitySubjectId}
  `;
  return rows[0];
}
...
function niosChapterDetail(title, place) {
  ...
  return `chapter: ${title} — ${place.subject_name} (${place.university_name})`;
}

router.post('/', auth, async (req, res, next) => {
  try {
    const { nios_university_subject_id, title, description, chapter_order } = req.body;
    if (!nios_university_subject_id || !title) return res.status(400).json({ error: 'nios_university_subject_id and title are required.' });
    const rows = await sql`
      INSERT INTO nios_chapters (nios_university_subject_id, title, description, chapter_order, created_by)
      VALUES (${nios_university_subject_id}, ${title}, ${description || null}, ${chapter_order || 1}, ${req.user.id})
      RETURNING *
    `;
    const place = await niosChapterPlace(nios_university_subject_id);
...
router.put('/:id', auth, async (req, res, next) => {
  try {
    ...
    `;
    if (!rows[0]) return res.status(404).json({ error: 'Not found.' });
    const place = await niosChapterPlace(rows[0].nios_university_subject_id);
