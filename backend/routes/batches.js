  ...
  const rows = university_id
    ? all
      ? await sql`
          ...
          WHERE b.university_id = ${university_id} ORDER BY b.name`
      : await sql`
          SELECT b.*, u.name AS university_name, s.name AS stream_name
          FROM batches b
          LEFT JOIN universities u ON u.id = b.university_id
          LEFT JOIN streams s ON s.id = b.stream_id
          WHERE b.university_id = ${university_id} AND b.is_active = true ORDER BY b.name`
    : all
      ? await sql`
          SELECT b.*, u.name AS university_name, s.name AS stream_name
          FROM batches b
...
router.post('/', auth, async (req, res) => {
  ...
  const { name, university_id, stream_id, semester } = req.body;
  ...
  const rows = await sql`
    ...
    VALUES (${name}, ${university_id}, ${stream_id || null}, ${semester || null})
    RETURNING *
  `;
...
router.put('/:id', auth, async (req, res) => {
  ...
  if (!name || !university_id) return res.status(400).json({ error: 'Name and university are required.' });
  const rows = await sql`
    UPDATE batches SET name = ${name}, university_id = ${university_id},
      stream_id = ${stream_id || null}, semester = ${semester || null}
