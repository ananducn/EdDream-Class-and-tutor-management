...
function EmptyState({ children = 'No data for the selected period.' }) {
...
function useReport(endpoint, { dated = true } = {}) {
  ...
  async function run(dateFrom, dateTo) {
    ...
    setLoading(true);
    try {
      const res = await client.get(endpoint, { params: dated ? { date_from: dateFrom, date_to: dateTo } : {} });
      setData(res.data);
    } catch (err) {
...
// Recording progress is a snapshot of the whole syllabus, not a date-ranged
// count of sessions, so this tab has no date pickers.
function RecordingsTab() {
  const { data, loading, run } = useReport('/reports/recordings', { dated: false });
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-slate-400">
        Snapshot of the whole syllabus. Totals count each chapter once, however many streams share its subject.
      </p>
      {data && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <StatCard label="Total Chapters" value={data.total_chapters} tone="blue" />
            <StatCard label="Recorded"       value={data.total_recorded} tone="green" sub={pct(data.total_recorded, data.total_chapters) + ' complete'} />
            <StatCard label="Not Recorded"   value={data.total_not_recorded} tone="red" />
...
function UploadsTab() {
  const { data, loading, run } = useReport('/reports/uploads', { dated: false });
  const tick = (val) => (val ? '✓' : '—');
  const destinations = (r) =>
    [r.upload_student_app, r.upload_youtube, r.upload_gdrive, r.upload_harddisk].filter(Boolean).length;
  ...
  return (
    <div className="space-y-4">
      ...
      {data && (
        !data.length ? <EmptyState>No recordings yet.</EmptyState> : (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <TableBody>
                  {data.map((row) => (
                    <TableRow key={row.id} className={destinations(row) === 0 ? 'bg-amber-50/60 dark:bg-amber-950/20' : ''}>
                      ...
                      <TableCell className="text-center">{row.upload_youtube ? (row.youtube_privacy || '✓') : '—'}</TableCell>
            ...
            <p className="text-xs text-slate-400">Highlighted rows are recorded but not on any destination yet.</p>
          </>
        )
      )}
    </div>
...
// University → Stream → Subject. Streams come from the class's own stream_id, so
// unlike the recordings breakdown these rows do sum to the university total.
