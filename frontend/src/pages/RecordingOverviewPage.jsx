...
export default function RecordingOverviewPage() {
  ...
  // Load the recording report at whatever level is selected: a whole university
  // (all its streams), a stream, a year, or a semester.
  async function loadOverview(uni, str, year, sem) {
    let qs;
    if (str) {
      qs = `stream_id=${str}`;
      if (year) qs += `&academic_year_id=${year}`;
      if (sem) qs += `&semester_id=${sem}`;
    } else if (uni) {
      qs = `university_id=${uni}`;
    } else {
      setOverview([]);
      return;
    }
    setLoading(true);
    try {
      const ov = await client.get(`/chapter-recordings/overview?${qs}`);
      setOverview(ov.data);
  ...
  async function handleUni(v) {
    if (v === '__none') v = '';
    setSelUni(v); resetBelow('uni'); setSelSubject('');
  ...
  async function handleStream(v) {
    if (v === '__none') v = '';
    setSelStr(v); resetBelow('stream');
    if (!v) { loadOverview(selUni, '', '', ''); return; }
    setLoading(true);
    try {
      const ay = await client.get(`/academic-years?stream_id=${v}`);
      setAcademicYears(ay.data);
    } catch { toast.error('Failed to load stream syllabus.'); }
    loadOverview(selUni, v, '', '');
  }

  async function handleYear(v) {
    if (v === '__none') v = '';
    setSelYear(v); resetBelow('year');
    if (!v) { loadOverview(selUni, selStr, '', ''); return; }
    setLoading(true);
    try {
  ...
  async function handleSem(v) {
    ...
    loadOverview(selUni, selStr, selYear, v);
  }
  ...
  async function openEditorByChapter(row) {
    ...
    try {
      const res = await client.get(`/chapter-recordings/${row.chapter_id}`);
      openEditor({ ...(res.data || {}), chapter_id: row.chapter_id, chapter_title: row.chapter_title });
    } catch { toast.error('Failed to load recording.'); }
  }

  // Read-only details for a chapter. The per-subject table already carries the
  // recording columns, so it passes them in; report rows only have flags and need
  // a fetch.
  ...
  function editFromDetails() {
    ...
    openEditor({ ...(detailsRec || {}), chapter_id: meta.chapter_id, chapter_title: meta.chapter_title });
  }
  ...
  async function saveRecording(e) {
    ...
    try {
      ...
      setEditorOpen(false);
      // refresh the chapter list + the report at the current level
      if (selSubject) await handleSubject(selSubject);
      await loadOverview(selUni, selStr, selYear, selSem);
    } catch (err) {
  ...
  // otherwise they cover whatever level the report was loaded at.
  const scopedOverview = useMemo(
    () => (selSubject ? overview.filter((r) => String(r.academic_year_subject_id) === String(selSubject)) : overview),
    [overview, selSubject],
  );
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            <div className="space-y-1">
              ...
              <Select value={selUni} onValueChange={handleUni}>
                ...
                <SelectContent><SelectItem value="__none">None</SelectItem>{universities.map((u) => <SelectItem key={u.id} value={String(u.id)}>{u.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
              <Label>Stream</Label>
              <Select value={selStr} onValueChange={handleStream} disabled={filteredStreams.length === 0}>
                ...
                <SelectContent><SelectItem value="__none">None</SelectItem>{filteredStreams.map((s) => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
              ...
              <Select value={selYear} onValueChange={handleYear} disabled={academicYears.length === 0}>
                ...
                <SelectContent><SelectItem value="__none">None</SelectItem>{academicYears.map((ay) => <SelectItem key={ay.id} value={String(ay.id)}>{ay.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            {semesters.length > 0 && (
              <div className="space-y-1">
                ...
                <Select value={selSem} onValueChange={handleSem}>
                  ...
                  <SelectContent><SelectItem value="__none">None</SelectItem>{semesters.map((s) => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            )}
            <div className="space-y-1">
              ...
              <Select value={selSubject} onValueChange={handleSubject} disabled={subjects.length === 0}>
                ...
                <SelectContent><SelectItem value="__none">None</SelectItem>{subjects.map((s) => <SelectItem key={s.id} value={String(s.id)}>{s.subject_name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
        </CardContent>
      ...
      {/* Chapters — editable per-subject table, or a report at the selected level */}
      <Card>
        ...
        <CardContent>
          {selSubject ? (
                  ...
                  <TableBody>
                    {chapters.map((ch) => (
                      <TableRow
                        key={ch.chapter_id}
                        onClick={() => openDetails(
                          { chapter_id: ch.chapter_id, chapter_title: ch.chapter_title, chapter_order: ch.chapter_order, subject_name: selSubjectName },
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500 py-8 text-center">
              {overview.length === 0
                ? 'Select a university (and optionally a stream, year, or semester) to see its recording report.'
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    {!selStr && <TableHead className={TH}>Stream</TableHead>}
                ...
                <TableBody>
                  {overview.filter((r) => r.chapter_id).map((r) => (
                    <TableRow key={r.chapter_id} onClick={() => openDetails(r)} className="cursor-pointer">
                      {!selStr && <TableCell className="text-sm text-slate-600 dark:text-slate-300">{r.stream_name}</TableCell>}
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {detailsLoading ? (
            ...
            <div className="space-y-4">
              <div>
                {detailsMeta?.stream_name && <DetailRow label="Stream">{detailsMeta.stream_name}</DetailRow>}
                <DetailRow label="Subject">{detailsMeta?.subject_name}</DetailRow>
                {detailsMeta?.academic_year_name && <DetailRow label="Year">{detailsMeta.academic_year_name}</DetailRow>}
                {detailsMeta?.semester_name && <DetailRow label="Semester">{detailsMeta.semester_name}</DetailRow>}
        ...
        <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          {statRows.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {statRows.map((s) => (
                  <TableRow key={s.academic_year_subject_id}>
                    {!selStr && <TableCell className="text-sm text-slate-600 dark:text-slate-300">{s.stream_name}</TableCell>}
